...
(() => {
  ...
  function exactFeedToMainSales(order){
    ...
    // Capture before/after main DB sale count to verify the app accepted the sale.
  ...
  function exportData(){
    return {module:'gengrail-ebay',version:3,exportedAt:nowISO(),data:clone(state)};
  }
  ...
  function render(){
    ...
    const listings=state.listings.slice(0,5).map(x=>`
      <div class="ge-card">
    ...
    const sales=state.orders.slice(0,5).map(x=>`
      <div class="ge-card">
