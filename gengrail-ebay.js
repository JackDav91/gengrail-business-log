...
(() => {
  ...
  function getSummary(){
    ...
    return {
      ...
      drafts:state.listings.filter(x=>x.status==='DRAFT').length,
      lastSync:state.connection.lastSync
    };
  }

  function upsertFromStock(data={}){
    if(!data.inventoryId) throw new Error('inventoryId is required for Stock-driven eBay drafts.');
    let listing=state.listings.find(x=>String(x.inventoryId)===String(data.inventoryId));
    if(listing){
      Object.assign(listing,{
        ...
        photoCount:num(data.photoCount),
  ...
  function render(){
    ...
    if(!root){
      ...
      document.querySelector('.app')?.appendChild(root) || document.body.appendChild(root);
    }
    ...
    const listings=state.listings.slice(0,10).map(x=>`
      <div class="ge-card">
        <div class="ge-card-top"><div><div class="ge-card-title">${esc(x.title)}</div><div class="ge-card-sub">${esc(x.sku)} · ${x.source==='stock'?'from Stock':'linked to Stock'}${x.photoCount?' · '+x.photoCount+' photos':''}</div></div><div class="ge-status">${esc(x.status)}</div></div>
        <div class="ge-row"><div><span>COST</span><b>${money(x.purchaseCost)}</b></div><div><span>${x.status==='DRAFT'?'TARGET':'LISTED'}</span><b>${money(x.listPrice)}</b></div><div><span>CHANNEL</span><b>eBay</b></div></div>
    ...
    root.innerHTML=`
      <div class="ge-panel">
        ...
        <div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap"><span class="ge-badge ${linked?'ok':''}">${linked?'MAIN LOG LINKED ✓':'MAIN LOG NOT DETECTED'}</span><span class="ge-badge">${s.drafts} EBAY DRAFT${s.drafts===1?'':'S'}</span></div>
